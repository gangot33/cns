...
public class PlayfairCipher {
    ...
    static boolean[] visited = new boolean[26];  // Track if a character has been used in the key
    ...
    static void generateKeyMatrix(String key) {
        ...
        for (int i = 0; i < 26; i++) visited[i] = false;  // Reset visited array

        // First, add characters from the key
        for (int i = 0; i < key.length(); i++) {
            char ch = key.charAt(i);
            if (ch == 'j') ch = 'i';  // Treat 'j' as 'i'
            if (Character.isAlphabetic(ch) && !visited[ch - 'a']) {
        ...
        // Then, add the remaining characters of the alphabet
        for (char ch = 'a'; ch <= 'z'; ch++) {
            if (ch == 'j') continue;  // Skip 'j' since it's treated as 'i'
            if (!visited[ch - 'a']) {
                keyMatrix[k / 5][k % 5] = ch;
                visited[ch - 'a'] = true;
                k++;
    ...
    // Format the text by replacing 'j' with 'i' and ensuring an even length
